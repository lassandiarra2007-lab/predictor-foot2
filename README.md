...
<head>
    ...
    <title>Prédicteur Football Pro v3.2</title>
...
<body>
    <div class="container">
        <header>
            <h1>⚽ PRÉDICTEUR FOOTBALL PRO <span class="version-badge">v3.2</span></h1>
            <p class="subtitle">Analyse Avancée - xG Réels + Pondération Temporelle + Absences Joueurs Clés</p>
        </header>

        <div class="main-grid">
            <div class="card">
                <div class="card-title">🏠 Équipe Domicile</div>
                <input type="text" class="team-name-input" id="teamA-name" placeholder="Nom de l'équipe à domicile">

                <div class="quick-fill-section">
                    <button class="quick-fill-btn" onclick="quickFillTeamA()">📋 Remplir démo</button>
                </div>

                <div class="level-section">
                    <div class="level-title">📊 Niveau de l'équipe</div>
                    <div class="level-inputs">
                        ...
                        <div class="input-group">
                            ...
                            <select id="teamA-level">
                                <option value="10">10 - Élite mondiale</option>
                                <option value="9">9 - Top européen</option>
                ...
                <div class="form-section">
                    <div class="section-label">État de forme actuel</div>
                    <div class="form-state-selector" id="teamA-form"></div>
                </div>

                <div class="rest-days-section">
                    <div class="rest-days-title">⏰ Jours de repos avant le match</div>
                    <input type="number" id="teamA-rest-days" min="1" max="14" value="7" class="rest-days-input">
                </div>

                <div class="form-section">
                    <div class="section-label">📋 5 Derniers Matchs (xG RÉELS)</div>
                    <div id="teamA-matches"></div>
                </div>

                <div class="form-section">
                    <div class="section-label">🏠 3 Derniers Matchs À DOMICILE (xG RÉELS)</div>
                    <div id="teamA-home-matches"></div>
                </div>

                <div class="absence-section">
                    <div class="section-label">🚑 Absences Joueurs Clés</div>
                    <div class="absence-grid">
                        <div class="absence-column">
                            <div class="absence-column-title attack">⚔️ ATTAQUE</div>
                        ...
                        <div class="absence-column">
                            <div class="absence-column-title defense">🛡️ DÉFENSE</div>
            ...
            <div class="card">
                <div class="card-title">✈️ Équipe Extérieur</div>
                <input type="text" class="team-name-input" id="teamB-name" placeholder="Nom de l'équipe à l'extérieur">

                <div class="quick-fill-section">
                    <button class="quick-fill-btn" onclick="quickFillTeamB()">📋 Remplir démo</button>
                </div>
                ...
                <div class="form-section">
                    <div class="section-label">✈️ 3 Derniers Matchs À L'EXTÉRIEUR (xG RÉELS)</div>
        ...
        <div class="card">
            <div class="card-title">🔄 3 Derniers Matchs Face à Face</div>
            <div class="h2h-section">
                <div class="h2h-inputs">
                    <div class="h2h-match">
                        <div class="h2h-match-label">Match 1 (plus récent)</div>
            ...
            <div class="disparity-indicator" id="disparity-display">
                <div class="disparity-label">Écart de niveau calculé</div>
                <div class="disparity-value balanced" id="disparity-value">Équilibré</div>
            </div>
        ...
        <button class="predict-btn" id="predict-btn" onclick="generatePredictions()">🎯 GÉNÉRER LES PRÉDICTIONS</button>
        <div id="results" class="results hidden"></div>
    </div>

    <script>
        const FORM_STATES = {
            S: { label: 'S', description: 'Très bonne forme', points: 5 },
            ...
            E: { label: 'E', description: 'Très mauvaise forme', points: 1 }
        };

        const PLAYER_IMPACTS = {
            light: { label: 'Léger', impact: 0.05 },
        ...
        const CONTEXTS = {
            ...
            equilibre: { label: 'Équilibré', modifier: 1.0 },
            desequilibre: { label: 'Déséquilibré', modifier: 0.85 }
        };
        ...
        function addAbsencePlayer(team, type) {
            ...
            item.innerHTML = `
                ...
                <div class="absence-impact-selector">
                    <button class="absence-impact-btn light active" onclick="setAbsenceImpact('${itemId}', 'light', this)">Léger</button>
        ...
        function updateDisparityDisplay() {
            ...
            if (disparityData.disparity > 0.15) {
                ...
                disparityValue.textContent = `Avantage Extérieur (+${(Math.abs(disparityData.disparity) * 100).toFixed(0)}%)`;
                disparityValue.className = 'disparity-value favor-b';
            } else {
                disparityValue.textContent = 'Match équilibré';
        ...
        function generatePredictions() {
            const teamAName = document.getElementById('teamA-name').value || 'Équipe Domicile';
            const teamBName = document.getElementById('teamB-name').value || 'Équipe Extérieur';

            if (!appState.teamA.form || !appState.teamB.form) {
                showCustomAlert('⚠️ Veuillez sélectionner l\'état de forme des deux équipes');
                return;
            }
            ...
            if (maxProb >= 70) {
                if (probWinA >= 70) {
                    predictions.push({ label: 'Résultat', value: `Victoire ${teamAName}`, confidence: probWinA });
                } else if (probWinB >= 70) {
                    predictions.push({ label: 'Résultat', value: `Victoire ${teamBName}`, confidence: probWinB });
                } else if (probDraw >= 70) {
                    predictions.push({ label: 'Résultat', value: 'Match Nul', confidence: probDraw });
                }
        ...
        function displayResults(teamAName, teamBName, xgA, xgB, xgaA, xgaB, xgA_Final, xgB_Final, predictions, probableScores, stats) {
            ...
            html += '<div class="result-title">📊 ANALYSE & PRÉDICTIONS</div>';
            ...
            html += '<div class="xg-title">⚽ Expected Goals MARQUÉS (Pondérés)</div>';
            ...
            html += '<div class="xg-comparison">';
            html += '<div class="xg-title">🛡️ Expected Goals CONCÉDÉS (Pondérés)</div>';
            html += '<div class="xg-row">';
            ...
            html += '<div class="xg-title">🎯 xG FINAUX POISSON</div>';
            html += '<div class="xg-row">';
            ...
            html += '<div class="xg-comparison">';
            html += '<div class="xg-title">🚑 Absences Joueurs Clés</div>';
            html += '<div class="xg-row">';
            ...
            html += `<div class="xg-item"><div class="xg-label">${teamAName} - Défense</div><div style="font-size: 0.9rem; color: var(--accent-blue);">${absencesA_Defense}</div></div>`;
            ...
            html += `<div class="xg-item"><div class="xg-label">${teamBName} - Défense</div><div style="font-size: 0.9rem; color: var(--accent-blue);">${absencesB_Defense}</div></div>`;
            html += '</div></div>';

            html += '<div class="xg-comparison">';
            html += '<div class="xg-title">📊 Analyse Disparité de Niveau</div>';
            html += '<div class="xg-row">';
            ...
            const disparityText = disparityData.disparity > 0.15
                ...
                : disparityData.disparity < -0.15
                    ? `Avantage ${teamBName}`
                    : 'Match équilibré';
            html += `<div class="xg-row"><div class="xg-item" style="grid-column: span 2;"><div class="xg-label">Verdict disparité</div><div class="xg-value">${disparityText} (${(Math.abs(disparityData.disparity) * 100).toFixed(0)}%)</div></div></div>`;
            html += '</div>';

            html += '<div class="xg-comparison" style="border-color: var(--accent-green);">';
            html += '<div class="xg-title">✅ PRÉDICTIONS VALIDÉES</div>';
            if (predictions.length === 0) {
                html += '<div class="no-predictions">⚠️ Aucune prédiction ne respecte les seuils de certitude.<br><br>Seuils: Victoire/Nul ≥70% | BTTS 80-95% | Over/Under ≥75%</div>';
            } else {
            ...
            html += '<div class="xg-comparison">';
            html += '<div class="xg-title">🎯 3 Scores Les Plus Probables</div>';
